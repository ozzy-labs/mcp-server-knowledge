...
1 つのリポジトリを Claude Code / Codex CLI / Gemini CLI / GitHub Copilot CLI のいずれでも扱えるようにする設計指針。拡張機構そのものの仕様は `standards/agent-extensions.md`、共通指示ファイルは `standards/agents-md.md` を参照。
...
- **`.mcp.json` は Claude Code が読む名前だが、Gemini/Copilot も類似パスを参照可能** — 詳細は `standards/mcp-protocol.md`
...
> **注意**: Windows では symlink 作成に admin 権限または developer mode が必要。代替として `mklink /J` のジャンクションを使うか、共通 skill をプラグイン化して配布する（`standards/agent-extensions.md` の Plugins 節）。
...
- 本リポジトリの `standards/agent-extensions.md` — 拡張機構の横断仕様
- 本リポジトリの `standards/agents-md.md` — `AGENTS.md` の書き方
- 本リポジトリの `standards/ai-context-management.md` — 指示ファイルの肥大化対策
- 本リポジトリの `standards/mcp-protocol.md` — MCP サーバー設計
- 本リポジトリの `standards/prompt-injection.md` — Skills / Hooks のセキュリティ
