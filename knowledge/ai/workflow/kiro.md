...
AWS が提供する spec-driven development（SDD）ベースのエージェント型 AI IDE。ブラウザで動く Kiro Web IDE、デスクトップ配布のエディタ（[kiro.dev/downloads](https://kiro.dev/downloads/)）、ヘッドレスで走る Kiro CLI の 3 形態がある（2025-11-17 GA 以降のメッセージングは Web IDE 中心）。「prototype から production まで spec 駆動で持っていく」を看板に掲げ、Cursor / Claude Code 系の対話型コーディングとは異なるアプローチを取る。
...
CLI 2.0（2026-04-13）で Windows 11 と headless CI/CD 実行に対応（`KIRO_API_KEY` + `--no-interactive`）。CLI 2.1（2026-04-24）で shell output streaming・Tool Search・Skills slash command・device flow auth・RHEL サポート、CLI 2.2.0（2026-04-27）で adaptive thinking と subagent dispatch 修正、CLI 2.3.0（2026-05-12）で `KIRO_HOME` env var / V2 TUI keybindings 設定 / DCR 非対応 MCP サーバー向け OAuth Client ID / `$AGENT_DISPLAY_OUT` `$AGENT_CONTEXT_OUT` の agent output side channels が追加。CLI 2.4.0（2026-05-20）で会話の rewind / `/effort`（low–max の reasoning effort）/ `/settings` 統合メニュー、CLI 2.5.0（2026-05-29）で thinking display・subagent review loop・`/settings display`、CLI 2.6.0（2026-06-05）で transcript export（`/transcript save`）・terminal window title（`/title`）・`chat --effort` フラグ・model/effort preference の永続化が追加。IDE と CLI は同じ spec ファイルを共有する。バージョン履歴は [kiro.dev/changelog/cli](https://kiro.dev/changelog/cli/) を参照。

## 動作モデル

- **モデル**: Free tier の既定は Claude Sonnet 4.5。有料帯では Claude Sonnet 4.6、Claude Opus 4.6 / 4.7 / 4.8（4.8 が最新、1M context・128K max output・2.2x credit multiplier）、`Auto`（複数 frontier モデルを自動選択）が選択可能。open-weight モデルは Qwen3 Coder Next / DeepSeek v3.2 / MiniMax M2.5（M2.1）/ GLM-5 で Free tier でも利用可
...
- `--trust-all-tools` — 全ツールの承認をスキップ
- `--trust-tools=<categories>` — `read` / `grep` / `write` のカテゴリ単位で承認スキップ
- `--require-mcp-startup` — MCP 起動失敗時に即終了

**注意**: Headless での API key 認証は **Pro / Pro+ / Power サブスクライバー限定**（Free 不可）。詳細は [kiro.dev/docs/cli/headless](https://kiro.dev/docs/cli/headless)。

詳細は [kiro.dev/docs/cli](https://kiro.dev/docs/cli/) を参照。

## 料金

[kiro.dev/pricing](https://kiro.dev/pricing/) に最新プラン。GA（[2025-11-17 アナウンス](https://kiro.dev/blog/general-availability/)）後も Free（$0/月、50 credits/月、open-weight モデル [Qwen3 Coder Next / DeepSeek v3.2 / MiniMax 2.1] + Claude Sonnet 4.5）が永続提供される。有料プランは Pro $20/月（1,000 credits、Auto / Claude Sonnet 4.6 / Claude Opus 4.6 等）／ Pro+ $40/月（2,000 credits）／ Power $200/月（10,000 credits）。超過分は $0.04/credit、AWS GovCloud は約 20% 高。新規ユーザーが social login / Builder ID で初回有料アップグレード時に **$20 サインアップクレジット**、Students tier も提供（2026-03-18 開始）。Per-prompt クレジット消費が IDE 内で可視化される。
...
7. **GA 後も CLI が高速に進化していることを忘れる** — CLI は 2026-04 に 2.0、以降ほぼ週次ペースで 2.6.0（2026-06）まで機能追加が続いている。プロダクション CI に組み込む場合は version pin 必須
