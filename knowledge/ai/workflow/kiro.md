...
CLI 2.0 で Windows と headless CI/CD 実行に対応。IDE と CLI は同じ spec ファイルを共有する。

## 動作モデル

- **モデル**: 既定で Claude Sonnet 4.5。`Auto` モードでは複数の frontier モデルを自動選択
...
[kiro.dev/pricing](https://kiro.dev/pricing/) に最新プラン。Public preview 期間は無料枠あり、有料プランは月額制（2026-05 時点で最低 $20/月）。Per-prompt クレジット消費が IDE 内で可視化される。

認証は AWS Builder ID / IAM Identity Center / GitHub / Google アカウントに対応。
...
7. **Public preview の安定度を GA 前提で扱う** — 仕様変更が起こりうる。プロダクション CI に組み込む場合は version pin 必須
