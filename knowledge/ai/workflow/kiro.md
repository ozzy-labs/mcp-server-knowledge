...
CLI 2.0（2026-04-13）で Windows 11 と headless CI/CD 実行に対応（`KIRO_API_KEY` + `--no-interactive`）。続く CLI 2.1（2026-04-24）で shell output streaming・Tool Search・Skills slash command・device flow auth・RHEL サポートが追加。IDE と CLI は同じ spec ファイルを共有する。
...
[kiro.dev/pricing](https://kiro.dev/pricing/) に最新プラン。GA（2025-11-17）後も Free（$0/月、50 credits/月）が永続提供される。有料プランは Pro $20/月（1,000 credits）／ Pro+ $40/月（2,000 credits）／ Power $200/月（10,000 credits）。超過分は $0.04/credit、AWS GovCloud は約 20% 高。Per-prompt クレジット消費が IDE 内で可視化される。
