...
| `/plan` | プラン作成・編集 |
| `/status` | セッションステータス表示 |
| `/usage` | アカウントのトークン使用量・rate-limit リセット表示（v0.140+） |
| `/import` | Claude Code の設定・プロジェクトファイル・最近のチャットをインポート（v0.140+） |
| `/delete` | 現在のセッションを完全削除して終了（v0.140+） |
...
- ChatGPT 有料プランが必要
- Windows はネイティブ対応（PowerShell + Windows サンドボックスで実行）。Linux-native 環境が必要な場合は WSL2 を使う

## システム要件

- macOS 12+, Ubuntu 20.04+/Debian 10+, Windows 11（PowerShell でネイティブ実行、または WSL2）
