...
- **Behavior**: `Auto-fix pull requests`（bool のみ）。GitHub trigger で PR レビューに反応するルーチンが指摘箇所を自動修正してコミットするか
- **Permissions**: `Allow unrestricted git push`（bool のみ、既定オフ、後述）
...
デフォルトは `claude/`-prefix のブランチにのみ push 可能（保護ブランチ誤改変防止）。既存ブランチに push したい場合は ルーチンの **Permissions → Allow unrestricted git push** を有効化する必要がある。

### Behavior

`Auto-fix pull requests`（bool のみ）。GitHub trigger で PR レビューに反応するルーチンが指摘箇所を自動修正してコミットするかを制御する。レビューのみで修正は人間に任せたい場合は OFF。
...
5. **既存 main ブランチへの直接 push を期待** — デフォルトは `claude/`-prefix のみ。明示的に **Allow unrestricted git push** が必要
...
12. **`Behavior → Auto-fix pull requests` を有効化したまま放置** — GitHub trigger で PR レビューに反応するルーチンに有効化されていると、レビュー指摘ごとに自動修正コミットが乗る。レビューのみ・修正は人間がやりたい場合は明示的に OFF
