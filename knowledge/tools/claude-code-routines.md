...
8. **pre-commit hook の install を忘れる** — fresh clone なので `node_modules` と git hooks は未インストール。lefthook 等を使うリポは setup script で `pnpm install`（postinstall で hook が登録される）か、SessionStart hook を repo に commit して install を再現する
