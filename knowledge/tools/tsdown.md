...
- 2026-05 時点で v0.21.10（最新）。v0.22.0 はベータ進行中。**1.0 未満で minor 更新に破壊的変更がある**ため、バージョンは厳密に固定する
...
Node 20.19 以上が必要（ただし 22.18.0 未満は deprecated 扱いで、次期 minor で削除予定。22.18.0 以降推奨）。
...
`package.json` の `types` / `typings` フィールドが設定されていれば `--dts` は自動で有効になる。
...
4. **`^0.21.0` で固定して CI が壊れる** — 0.x はマイナー間で破壊的変更を許容するため `^` を使わない（実際 v0.22 ベータが進行中）。
