...
| 記事 | reviewed | stability | tags | 概要 |
|---|---|---|---|---|
| [cc-sdd](ai/workflow/cc-sdd.md) | 2026-05-03 | `beta` | `ai-workflow` `spec` `oss` `npm` `javascript` | OSS の SDD オーケストレータ（gotalab/cc-sdd）。npm パッケージとして配布され、`npx cc-sdd@latest` 一発で **17 個の Agent Skill**（discovery / spec / impl / steering 等）を 8 種類の AI コーディングエージェントに対してインストールする。「Kiro-inspired」を明言しており、Kiro の spec 形式（EARS + design + tasks）と互換。 |
