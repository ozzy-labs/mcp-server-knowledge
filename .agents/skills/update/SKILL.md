...
6. 既存 open PR が `staleness/<routine>-*` または `staleness/<article-slug>-*` で立っている記事は衝突回避でスキップ:

   ```bash
   gh pr list --state open --search "head:staleness/" --json headRefName
   ```
...
- **ブランチ**: `staleness/<article-slug>-<YYYYMMDD>`（slug は `ai-agents-gemini-cli` のように slash → hyphen に変換）
...
- **ブランチ**: `staleness/<routine>-<YYYYMMDD>`（例: `staleness/monthly-20260504`）
