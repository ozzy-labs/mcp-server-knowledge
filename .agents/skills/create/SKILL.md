...
- **カテゴリヒント**（任意）: `--category <path>` でカテゴリを明示指定（例: `tools`, `ai/agents`, `ai/practice`）
...
| カテゴリ | 対象 |
|---|---|
| `tools/` | **言語非依存の**汎用 CLI / lint / format / package（例: jq, mise, lefthook, trivy。**特定言語専用は `languages/<lang>/`、特定プラットフォーム専用は `platforms/<platform>/` へ**） |
...
1. **AI 関連** → `ai/<sub>/`
2. **言語専用** → `languages/<lang>/`
3. **プラットフォーム専用** → `platforms/<platform>/`
4. **規約・設計** → `standards/`
5. **言語非依存の汎用ツール** → `tools/`
